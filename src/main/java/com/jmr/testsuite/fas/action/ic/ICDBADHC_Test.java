...
public class ICDBADHC_Test extends AbstractTestNGSpringContextTests
{
	@Autowired
		...
		for (ICDBADHC_Page data : dataList) {
			try {
				...
				if (data.getRunMode().equalsIgnoreCase("Yes") || data.getRunMode().equalsIgnoreCase("Y")) {
					...
					fcubs.clickNew();
					fcubs.populateTextById("BLK_TDP_ADV__CUST_AC_NO", data.getAccountnumber());
					fcubs.populateTextById("BLK_TDP_ADV__STOCK_CATLG_CD", data.getStockCatalogCode());
					fcubs.populateTextById("BLK_TDP_ADV__CERTIFICATE_NO", data.getCertificateNumber());
					fcubs.selectFlag("BLK_TDP_ADV__DUPLICATE_FLAG", data.getMarkAsDuplicate());
					fcubs.selectFlag("BLK_TDP_ADV__AUTOGEN_CERT_NO", data.getAutoGenerateCertificateNo());
					Thread.sleep(3000);
					fcubs.clickButtonById("BLK_TDP_ADV__BTN_GENERATE");
					try {
