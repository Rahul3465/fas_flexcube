...
public class CSDEVENT_Test extends AbstractTestNGSpringContextTests
{
	@Autowired
	...
	private final String screenId = "CSDEVENT";
	private final String screenName="Events";

	private List<CSDEVENT_Page> dataList;
	private List<CSDEVENT_Page> resultList;
	...
	public void initSetup() throws Exception {
		...
		dataList = fcubs.loadDataFromExcel(CSDEVENT_Page.class, screenId);
	...
	public void executeTestCase() throws UIFormatException, Exception {
		...
		for (CSDEVENT_Page data : dataList) {
			try {
				...
				if (data.getRunMode().equalsIgnoreCase("Yes") || data.getRunMode().equalsIgnoreCase("Y")) {
					...
					Thread.sleep(2000);
					fcubs.populateTextById("BLK_EVENT__REFNO", data.getReferenceNumber());			// Reference Number
...
//		fcubs.destroy();
	...
	public void tearDown(ITestResult result) {
		...
		for (CSDEVENT_Page data : resultList) {
